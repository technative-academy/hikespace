...
import { useFollowingPosts, usePosts } from "@/features/post";
...
function Home() {
  ...
  const { posts: followingPosts, isLoading: followingLoading } = useFollowingPosts(!guest);
            ...
            {guest
              ? (
                <div className={styles.guestMessage}>
                  ...
                  <p>
                    This is a social app that lets you share your hiking adventures with the world.
                  </p>
                  <p>
                    Sign up or log in to see the full feed and follow other hikers.
                  </p>
                  <div
                  ...
                  <h4>Preview:</h4>
                  <Grid minWidth="12rem">
                    {posts.slice(0, 3).map((post) => <FeedPost key={post.id} post={post} />)}
                  </Grid>
                </div>
              )
              : (
            ...
            {guest
              ? (
                <div style={{ textAlign: "center", padding: "3rem" }}>
                  <h3>
                    Dear Guest user, Please kindly do log in to see your following feed
                  </h3>
                  <Button
              ...
              : (
                <Grid minWidth="12rem">
                  {followingPosts.map((post) => <FeedPost key={post.id} post={post} />)}
                </Grid>
              )}
          </TabsContent>
        </Tabs>
